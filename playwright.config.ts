...
// chrome83 project 전용 build+preview 서버(D7, 엔트리 2개: demo 4174 +
// showcase 4175, Issue #180). Playwright는 --project 필터와 무관하게
// webServer 배열 전체를 항상 띄운다(실측 확인) — 포트만 다르다고
// 격리되지 않는다. 이 엔트리를 무조건 배열에 두면 `pnpm
// test:e2e`(chromium 게이트)/`pnpm test:e2e:full`(3-엔진)/`pnpm
// test:e2e:perf`(성능 측정) — 모두 chrome83과 무관 — 도 매번 demo·showcase
// 프로덕션 빌드 + `vite preview` 기동을 떠안는다(트랙-6 발견 F1).
...
const chrome83WebServer = process.env.GEUL_CHROME83_WEBSERVER
  ? [
      ...
      {
        // showcase(Issue #180) build+preview. 자체 react build를 하지
        // 않는다 — 위 demo 엔트리가 만드는 packages/react/dist/styles.css
        // 마커를 기다린다(아래 일반 webServer의 showcase 엔트리와 동일
        // 패턴, 그 주석의 레이스 회피 이유를 그대로 재사용).
        command:
          "until [ -f packages/react/dist/styles.css ]; do sleep 0.3; done; pnpm --filter @cp949/geul-showcase build && pnpm --filter @cp949/geul-showcase exec vite preview --port 4175 --host 127.0.0.1 --strictPort",
...
export default defineConfig({
  ...
  projects: [
    ...
    },
    {
      ...
      // D10·D11). `@core` 시나리오 4개(demo: 링크 툴바, 블록 분리, 표 행
      // 추가 / showcase: 이미지 정렬, Issue #180)만 선택하고, dev 서버가
      // 아니라 build+preview 산출물을 서빙한다(D7) — vite dev는
      // build.target을 적용하지 않아 downlevel 결과가 반영되지 않는다.
      // `pnpm test:e2e:chrome83`(docker/chrome83 컨테이너 안에서만 실행)로만
      ...
      // 트랩 테스트 전용)에 합치지 않고 별도 파일로 뗐다(Issue #124). 이
      // 원칙에 예외가 하나 있다 — showcase-composite-media-alignment.spec.ts는
      // `@core` 3개(기본/Align left/Align right)를 한 파일에 그대로
      // 둔다. 3개 모두 같은 정렬 시나리오의 변형이라 Issue #124가
      // 막으려던 "무관한 test 편입"에 해당하지 않는다(01-계획.md
      // "## 결정" 참고). showcase 대상은 이 project의
      // `use.baseURL`(4174, demo 전용)을 쓰지 않는다 —
      // `e2e/support/showcase.ts`의 `SHOWCASE_BASE_URL`이
      // `GEUL_CHROME83_WEBSERVER` 존재 여부로 4175(build+preview)를 직접
      // 가리킨다.
      name: "chrome83",
      testMatch:
        /(link-toolbar|block-handle|table-keyboard-row-insert|showcase-composite-media-alignment)\.spec\.ts$/,
      grep: /@core/,
      use: {
