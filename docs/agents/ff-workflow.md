...
- 절차: subagent 협업 모드로 진행한다. 아래 렌즈 셋을 읽기 전용 subagent 3개에 하나씩 병렬 dispatch한다.
...
- 메인 세션이 세 보고를 합쳐 중복을 제거하고, 진짜 결함인지 직접 확인한 뒤 DELTA 파일을 수정한다. 계획서 내용 자체의 타당성 판정과 각 DELTA의 **참고 pitfalls** 절(형식은 "DELTA 계약"이 소유한다) 작성도 메인 세션이 한다 — 파일을 쓰는 것은 이 트랙에서 메인 세션뿐이다.
