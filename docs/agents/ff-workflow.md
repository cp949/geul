...
- 절차: subagent 협업 모드로 진행한다. 아래 렌즈 셋을 읽기 전용 subagent 3개에 하나씩 병렬 dispatch한다. **라운드마다 3개 전부를 다시 dispatch한다** — 수정된 DELTA를 처음부터 다시 본다.
...
- 메인 세션이 세 보고를 합쳐 중복을 제거하고, 진짜 결함인지 직접 확인한다. `BLOCKER`·`MAJOR`는 DELTA 파일을 수정한다. `MINOR`는 루프를 막지 않고 근거와 함께 `pending-issues/`로 분리한다 — 등록 기준은 [`./issue-tracker.md`](./issue-tracker.md)를 따른다. 계획서 내용 자체의 타당성 판정과 각 DELTA의 **참고 pitfalls** 절(형식은 "DELTA 계약"이 소유한다) 작성도 메인 세션이 한다 — 파일을 쓰는 것은 이 트랙에서 메인 세션뿐이다. 라운드가 끝날 때마다 `PLAN-REVIEW-NN`을 다음 번호로 append한다.
- 게이트: 그 라운드에서 `BLOCKER`·`MAJOR`가 0건이면 트랙을 마친다. 남아 있으면 수정한 DELTA로 다음 라운드를 시작한다. **3라운드를 넘겨도 남으면 자동 반복을 멈춘다** — 반복적으로 남는 결함을 보고하고 사용자 판단을 기다린다. 추측으로 계속 돌리지 않는다.
- 이 라운드 루프는 **한 번의 트랙-2 실행 안**에서 자동으로 돈다. 이미 끝난 트랙-2를 사용자가 나중에 다시 지시하는 "재실행"(위 "재실행" 절)과는 별개다.
