...
2. 현재 `ACTIVE`인 RD가 있고 그 RD의 "예상 DELTA" 백로그에 실행 가능한 다음 항목이 있으면 그것을 선택한다. 백로그 항목이 직전 DELTA가 남긴 한 줄 예상이면 직전 `result/RD-NNN-DELTA-NN.md`의 "## 결과" 절과 대조해 여전히 유효한지만 확인하고 처음부터 다시 분석하지 않는다 — 상황이 달라졌을 때만 그 자리에서 고쳐 쓴다. 다른 RD로 전환하지 않는다.
