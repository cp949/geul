...
- **예외 — qq-workflow.** 단계-1 계획 승인 이후 자동 진행하는 레인이다([`docs/agents/qq-workflow.md`](./qq-workflow.md)의 "진행 방식"). 단계-4에서 완료 조건이 전부 PASS이고 미해결 `FAIL`·`BLOCKER`·`MAJOR`가 없으면 위 게시 직전 확인 없이 등록 목록을 그대로 게시한다. 그렇지 않으면(완료 조건 미충족, 미해결 문제, 등록 판단이 서지 않음) 게시하지 않고 정지해 사용자에게 보고한다. ff-workflow와 기본 레인은 이 예외 대상이 아니다 — 위 게시 직전 등록 확인을 그대로 따른다. 이슈 종료는 레인과 무관하게 위 규칙대로 에이전트가 판정한다.
