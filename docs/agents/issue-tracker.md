...
에이전트는 사용자 지시 없이 GitHub에 쓰지 않는다(qq-workflow 예외는 아래 넷째 bullet 참고). 읽기는 제한하지 않는다 — `gh issue view`, `gh issue list`, `gh api` 조회는 언제든 실행한다.
...
- **예외 — qq-workflow.** 단계-1 계획 승인 이후 자동 진행하는 레인이다([`docs/agents/qq-workflow.md`](./qq-workflow.md)의 "진행 방식"). 단계-4에서 완료 조건이 전부 PASS이고 미해결 `FAIL`·`BLOCKER`·`MAJOR`가 없으면 위 게시 직전 확인 없이 등록 목록과 종료 예정 이슈를 그대로 게시·종료한다. 그렇지 않으면(완료 조건 미충족, 미해결 문제, 등록·종료 판단이 서지 않음) 게시하지 않고 정지해 사용자에게 보고한다. ff-workflow와 기본 레인은 이 예외 대상이 아니다 — 위 게시 직전 확인을 그대로 따른다.
...
닫을지는 에이전트가 스스로 판단한다. 근거는 완료 보고와 이슈 본문의 완료 기준이고, 사용자에게 되묻지 않는다. 사용자 확인을 받는 것은 "게시 승인"의 종료 예정 목록 제시 한 번뿐이다(qq-workflow는 예외 — "게시 승인"의 "예외 — qq-workflow" 참고, 조건이 맞으면 확인 없이 자동 종료한다).
