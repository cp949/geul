...
에이전트는 사용자 지시 없이 GitHub에 쓰지 않는다. workflow 완료 예외는 아래 넷째 bullet이 소유한다. 읽기는 제한하지 않는다 — `gh issue view`, `gh issue list`, `gh api` 조회는 언제든 실행한다.
...
닫을지는 에이전트가 스스로 판단하고, "게시 승인"을 통과한 같은 실행 안에서 수행한다. 근거는 완료 보고와 이슈 본문의 완료 기준이고, 사용자에게 되묻지 않는다. 종료에 별도 사용자 확인은 없다 — 확인 대상은 "게시 승인"의 등록 목록뿐이다.
