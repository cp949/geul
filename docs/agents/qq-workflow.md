...
간단한 작업을 계획서 하나와 병합 전 리뷰 1회로 처리하는 경량 작업 흐름이다. 단계 4개와 작업 브랜치, `_works/` 작업공간을 쓴다. 슬래시 커맨드는 없다 — 이 문서 하나가 절차를 소유하고 사용자가 단계를 지시한다.
...
진입 규칙은 [`../../AGENTS.md`](../../AGENTS.md)의 "레인 선택 규칙"이 소유한다. 이 흐름으로 들어오는 경로는 둘이다 — 사용자가 `qq-workflow`를 지목했거나, 레인 명시 없는 이슈 작업에서 자동 선택이 qq를 골랐다. 커맨드도 skill도 없다. 자동 선택으로 들어왔으면 선택한 레인과 이유를 작업 시작 시 사용자에게 보고한다.
...
단계 지시는 작업 폴더를 함께 받는다. 새 작업은 작업 설명만으로 시작한다.
...
ff-workflow의 `_meta.md`와 같은 형식에서 `완료 트랙`을 `완료 단계`로 바꾼 것이다.
...
"확정 커밋 해시" 절은 단계-4의 `dev` 이전이 끝난 뒤에만 채운다.

**단계 진입 가부는 `완료 단계` 문자열이 아니라 선행 산출물 파일의 존재로 판정한다.** 필요한 파일이 없으면 추측해서 진행하지 않고 무엇이 없는지 보고하고 정지한다.

**종료 기록.** 모든 단계는 끝날 때 `_meta.md`를 갱신한다 — `완료 단계`에 자기 번호를 넣고 `## 진행 로그`에 날짜와 한 줄 요약을 덧붙인다. 판단이 필요했던 지점은 `## 결정`에 남긴다.
