...
ff-workflow의 `_meta.md`와 같은 3필드 manifest다.
...
**재실행.** 단계-2와 단계-3은 이미 완료했어도 사용자가 지시하면 한 번 더 실행한다. 리뷰 산출물은 기존 파일을 덮어쓰지 않고 다음 번호로 append한다. 재실행이 게이트를 다시 밟게 하면 그 게이트도 다시 실행한다 — 단계-3 재실행은 `pnpm verify` 전량을 다시 돌린다.
...
- 출력: 작업 브랜치 커밋
- 절차: subagent 협업 모드로 진행한다. 메인 세션이 아래 "구현 prompt"를 파일 쓰기 subagent 하나에 전달한다. subagent가 반환하면 메인 세션은 diff와 검증 증거를 확인한 뒤 커밋한다.
