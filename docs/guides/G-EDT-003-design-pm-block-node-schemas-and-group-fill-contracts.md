...
- 그룹의 의도된 기본 채움 노드를 하나 정하고 그 노드의 priority를 그룹 내 최고로 명시한다(현재: `blockContainer` 1000). 새 그룹 멤버는 그보다 낮게 둔다.
...
- Tiptap(3.30.1 실측)에서 priority는 두 축을 따로 움직인다: priority가 높을수록 스키마 등록이 앞서 defaultType 경쟁에서 이기고, 같은 priority에서는 나중 등록 확장의 keymap이 먼저 실행된다(`sortExtensions(...).reverse()`). keymap 순서를 근거로 스키마 순서를 추정하지 않는다.
