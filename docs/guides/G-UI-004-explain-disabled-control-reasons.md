...
- 비활성 사유가 있으면 그 사유 문구를 `title`로 노출한다. 활성 상태에서는 `title`을 아예 생략한다(`undefined`를 넘긴다 — 빈 문자열은 "사유 없음"과 "빈 사유"를 구분하지 못한다).
...
- 활성 상태: `aria-disabled="false"`, `title` 속성이 없다(`getAttribute("title") === null`).
