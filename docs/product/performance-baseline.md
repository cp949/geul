...
위 표의 수치는 `perf` 프로젝트 분리(Issue #74) 이전에, 회귀 스위트와 같은 실행에서 6워커 경합 아래 측정한 값이다. 격리 후 재측정에서는 spec 자체의 wall-clock이 38.6초에서 13.4초로 줄었다 — 경합이 측정 대상 시간을 2.9배 부풀리고 있었다. 지표 중앙값은 같은 자릿수에 머물렀지만(로드 354.2ms, 붙여넣기 305.2ms, 선택 12.9ms, undo 14.8ms) 표본 조건이 달라졌으므로, 다음 의도된 재측정 시 `pnpm test:e2e:perf` 결과로 위 표를 통째로 교체한다. 그 전까지 위 값과 격리 실행 값을 직접 비교하지 않는다.
