...
| `QA-086` | 버튼 클릭 등 타이핑이 아닌 경로로 여는 메뉴도 정확한 anchor 위치에 뜬다(화면 구석에 고정되지 않는다) | `UI-002` | `FAIL` | 2026-09-08 사용자 실사용 중 발견(스크린샷) — 블록 거터의 Add block(+) 버튼을 클릭하면 새 블록 자리가 아니라 화면 좌측 상단 구석에 블록타입 선택 팝업(Text/Heading 1~5...)이 뜬다. `block-side-menu.tsx`의 `handleAddBlockClick` → `onBlockAdded` → `slash-menu.tsx:593` 경로로 추정 — 근본 원인 미확인(조사 중 사용자 지시로 체크리스트 작업으로 전환, 아직 수정 안 함) |
| `QA-011` | 코드 언어 콤보박스가 바로 다음 블록(trailing 빈 문단 등)을 가리지 않는다 | `BLK-011`      | `NOT_RUN` | 2026-09-08 사용자 실사용 중 발견(스크린샷) — `fix(react) edbb6b3`로 수정, unit test 8건 통과. **실브라우저 시각 확인 대기**(claude-in-chrome 미연결) |
| `QA-012` | 블록 거터(드래그 핸들·+)가 hover 이동 중 사라지지 않는다        | `UI-003`       | `NOT_RUN` | 2026-09-08 사용자 실사용 중 발견 — `fix(react) edbb6b3`로 수정, unit test 2건 통과. **실브라우저 시각 확인 대기**(claude-in-chrome 미연결) |
