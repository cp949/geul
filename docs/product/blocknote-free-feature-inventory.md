...
| `BLK-007` | 글머리 목록 | `PARITY` | R2 | `PARTIAL` | 슬라이스 5 RD-001·RD-002 완료(Issue #38) — 저장 타입·검증·canonicalization, core schema·codec·production load/save·공용 명령, flat HTML/GFM outbound와 load된 목록의 Enter/Backspace/Delete·Tab/Shift+Tab·marker·`List item` placeholder·Chromium 통합 구현. programmatic 변환 command는 RD-004, import·중첩 round-trip은 RD-003, 생성·변환 사용자 UX는 RD-005 잔여 |
| `BLK-008` | 시작 번호를 지원하는 번호 목록 | `PARITY` | R2 | `PARTIAL` | 슬라이스 5 RD-001·RD-002 완료(Issue #38) — 저장 타입·검증·canonicalization, `startNumber` `0..999999999`, core schema·codec·production load/save·공용 명령, flat HTML/GFM outbound와 형제 scope별 명시/자동 번호 표시·구조 편집·Chromium 통합 구현. programmatic 변환 command는 RD-004, import·중첩 round-trip은 RD-003, 생성·변환 사용자 UX는 RD-005 잔여 |
