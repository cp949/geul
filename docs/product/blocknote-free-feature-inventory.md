...
| `UI-011` | 기본 키보드 단축키와 입력 규칙 | `PARITY` | R2 | `PARTIAL` | 슬라이스 4 RD-004가 CodeBlock Tab 공백 2개·Shift+Tab focus escape와 CodeBlock 교차 mark shortcut 소비 후 no-op을, 슬라이스 5 RD-002가 목록 Enter/Backspace/Delete·Tab/Shift+Tab을, RD-006이 exact `- `·`1. ` native input rule과 즉시 Backspace 복원을 구현. 나머지 R2 keyboard shortcut·block input rule은 슬라이스 9 범위 |
