...
| `BLK-004` | 자식을 접는 토글 제목                | `PARITY`   | R2   | `VERIFIED`    | 슬라이스 6 RD-003·RD-004·RD-005 완료(Issue #38) — `HeadingBlock.isToggleable`/`collapsed` 저장·검증, children `blockGroup` 숨김·접힘 트라이앵글 decoration widget·`toggleHeadingCollapse` 명령·undo, Slash 메뉴 생성(레벨별)/Turn into, HTML(`<details data-be-toggleable>`+`<summary>`)·GFM(strict 거절·lossy `TOGGLE_STATE_LOST`, import는 토글 미생성) round-trip을 unit·Chromium gate로 검증 |
...
| `BLK-009` | 체크 목록                            | `PARITY`   | R2   | `VERIFIED`    | 슬라이스 6 RD-001·RD-002 완료(Issue #38) — `CheckListItemBlock` 저장·검증, 구조 편집·checked 클릭 decoration widget·`toggleCheckListItemChecked` 명령·undo, Slash 메뉴 생성/Turn into, HTML(`data-be-checked`)·GFM(`- [ ]`/`- [x]`) import·export·strict/lossy loss(중첩 정책·`CHECKED_STATE_LOST`)를 unit·Chromium gate로 검증                                                                  |
| `BLK-010` | 자식을 접는 토글 목록                | `PARITY`   | R2   | `VERIFIED`    | 슬라이스 6 RD-003·RD-004·RD-005 완료(Issue #38) — `ToggleListItemBlock` 저장·검증, children `blockGroup` 숨김·접힘 트라이앵글 decoration widget·`toggleListItemCollapse` 명령·undo, Slash 메뉴 생성/Turn into, HTML(`<details data-be-toggleable>`+`<summary>`)·GFM(strict 거절·lossy 일반 목록으로 낮춤+`TOGGLE_STATE_LOST`, import는 토글 미생성) round-trip을 unit·Chromium gate로 검증       |
