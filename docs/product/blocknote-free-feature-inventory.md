...
| `BLK-007` | 글머리 목록 | `PARITY` | R2 | `PARTIAL` | 슬라이스 5 RD-001·RD-002·RD-004 완료(Issue #38) — 저장 타입·검증·canonicalization, core schema·codec·production load/save·공용 명령, flat HTML/GFM outbound, load된 목록의 Enter/Backspace/Delete·Tab/Shift+Tab·marker·`List item` placeholder·Chromium 통합과 public `setBlockType` programmatic 변환 구현. import·중첩 round-trip은 RD-003, 생성·변환 사용자 UX는 RD-005 잔여 |
| `BLK-008` | 시작 번호를 지원하는 번호 목록 | `PARITY` | R2 | `PARTIAL` | 슬라이스 5 RD-001·RD-002·RD-004 완료(Issue #38) — 저장 타입·검증·canonicalization, `startNumber` `0..999999999`, core schema·codec·production load/save·공용 명령, flat HTML/GFM outbound, 형제 scope별 명시/자동 번호 표시·구조 편집·Chromium 통합과 `startNumber?: number | null` public `setBlockType` programmatic 변환 구현. import·중첩 round-trip은 RD-003, 생성·변환 사용자 UX는 RD-005 잔여 |
...
| `UI-011` | 기본 키보드 단축키와 입력 규칙 | `PARITY` | R2 | `PARTIAL` | 슬라이스 4 RD-004가 CodeBlock Tab 공백 2개·Shift+Tab focus escape와 CodeBlock 교차 mark shortcut 소비 후 no-op을, 슬라이스 5 RD-002가 load된 글머리·번호 목록의 Enter/Backspace/Delete와 Tab/Shift+Tab 소비를 구현. 목록 생성 input rule은 슬라이스 5 RD-005, 나머지 R2 keyboard shortcut·block input rule은 후속 슬라이스 범위 |
