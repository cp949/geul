...
| `BLK-007` | 글머리 목록 | `PARITY` | R2 | `PARTIAL` | 슬라이스 5 RD-001·RD-002·RD-004·RD-005·RD-006 완료(Issue #38) — 저장 타입·검증·canonicalization, core schema·codec·production load/save·공용 명령, flat HTML/GFM outbound, 구조 편집·marker·placeholder, Slash/Turn into/toolbar와 exact `- ` native input rule의 Chromium 통합 구현. import·중첩 round-trip은 RD-003 잔여 |
| `BLK-008` | 시작 번호를 지원하는 번호 목록 | `PARITY` | R2 | `PARTIAL` | 슬라이스 5 RD-001·RD-002·RD-004·RD-005·RD-006 완료(Issue #38) — 저장 타입·검증·canonicalization, `startNumber` `0..999999999`, core schema·codec·production load/save·공용 명령, 형제 scope별 명시/자동 번호 표시·구조 편집, Slash/Turn into/toolbar와 `startNumber` 없는 exact `1. ` native input rule의 Chromium 통합 구현. import·중첩 round-trip은 RD-003 잔여 |
