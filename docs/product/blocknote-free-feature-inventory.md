...
| `BLK-007` | 글머리 목록 | `PARITY` | R2 | `PARTIAL` | 슬라이스 5 RD-001(Issue #38) — 저장 타입·검증·canonicalization, core schema·codec·production load/save·공용 명령, flat HTML/GFM outbound 완료. 생성·변환·편집·React UX는 RD-002, import·중첩 round-trip은 RD-003 잔여 |
| `BLK-008` | 시작 번호를 지원하는 번호 목록 | `PARITY` | R2 | `PARTIAL` | 슬라이스 5 RD-001(Issue #38) — 저장 타입·검증·canonicalization, `startNumber` `0..999999999`, core schema·codec·production load/save·공용 명령, flat HTML/GFM outbound 완료. 생성·번호 표시·React UX는 RD-002, import·중첩 round-trip은 RD-003 잔여 |
