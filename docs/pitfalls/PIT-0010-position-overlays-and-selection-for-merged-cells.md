...
- `packages/core/src/table-commands.ts`의 `applyTableGridOperation` `options.selectCellId`, `findCellContentOffset` — 병합/분할 직후 결과 셀로 selection을 명시 이동.
- `packages/core/src/table-commands.ts`의 `options.preserveSelection`, `findCellBoundaryOffset` — 셀 서식 적용 전후 `CellSelection` anchor/head와 병합 셀 커서 `TextSelection` 보존.
