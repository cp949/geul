...
- 2026-08-22 — `.claude/commands/merge-dev.md` 4단계와 `PIT-0021` 검증 방법이 `git rebase -i dev`를 지시했다. 임시 저장소(커밋 3개)에서 `git rebase -i HEAD~2`를 실행하자 `Successfully rebased and updated refs/heads/main.`, exit code `0`, 커밋 해시 `7dfd0fc ea7a7fb f20872e` 전부 변화 없음이었다. `git var GIT_EDITOR`은 `true`, `core.editor`는 `vim`이었다.
...
- 이 규칙을 적용하는 커맨드: `.claude/commands/merge-dev.md`
