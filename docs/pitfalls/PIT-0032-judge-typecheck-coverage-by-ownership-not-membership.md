...
turbo는 package가 task script를 잃어도 dry graph의 항목을 없애지 않고 command를 `<NONEXISTENT>`로 표시한다. 실제 실행에서는 그 package가 빠져 실행 태스크 수가 줄어든다. 커버리지는 "이 파일이 속한 tsconfig 디렉터리에 그 파일이 직접 있고, 그 tsconfig가 실제로 컴파일 대상에 담는다"는 소유 기준으로 판정한다 — dry graph 항목 수, 실행 성공 또는 멤버십만으로 판정하지 않는다.
