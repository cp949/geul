...
- 오버레이 테스트는 fake DOM 대신 실제 `createEditor()` 마운트를 기본으로 한다(`packages/react/test/mount-editor.tsx`의 `mountTableEditor`/`mountBlockEditor`). 실제 ProseMirror view는 `contenteditable` **속성**을 세우므로(Issue #76에서 실측) 이 함정의 IDL/속성 괴리가 애초에 생기지 않는다. 다만 이 원칙이 기본값으로 자리 잡은 것은 층위 1의 5개 파일뿐이고, 그 안에서도 `table-selection-toolbar.test.tsx`는 18건 중 12건만 실제 마운트다. `link-toolbar`·`formatting-toolbar`·`table-cell-format-menu`·`editor-content`는 이번 이관 대상이 아니라 지금도 전부 fake 위에서 돈다 — 파일 수로는 오버레이 테스트 파일 9개 중 5개, 테스트 건수로는 약 124건 중 40건이 여전히 fake 레인이다. 아래 속성 대입 규칙은 그 40건 전체에 여전히 그대로 적용되고, 이 함정이 물러난 것은 층위 1 안의 실제 마운트 부분에 한정된다.
...
- Issue #76 — 층위 1의 5개 파일(`block-side-menu`, `slash-menu`, `table-handles`, `table-handle-menu`, `table-selection-toolbar`)을 실제 `createEditor()` 마운트로 옮겼다. 실제 마운트한 편집 영역을 실측하면 `getAttribute("contenteditable")`은 `"true"`, `hasAttribute("contenteditable")`은 `true`, `matches('[contenteditable="true"]')`는 `true`이고 `contentEditable` IDL 프로퍼티는 `undefined`다(jsdom이 IDL을 구현하지 않는 쪽이라 오히려 속성만 남는다). 즉 프로덕션 셀렉터가 테스트에서도 그대로 매치되므로 초점 복구가 no-op가 될 여지가 없다. 같은 실행에서 fake 경로(`document.createElement("div")` + `contentEditable = "true"` IDL 대입)는 여전히 `getAttribute`가 `null`, 셀렉터 매치가 `false`였다 — 위 규칙은 실제 마운트가 불가능한 테스트에만 계속 유효하다. 마운트 헬퍼는 `packages/react/test/mount-editor.tsx`, 편집 영역 획득은 `packages/react/test/query-mounted-editable.ts`의 `queryMountedEditable`이고, 속성 계약 자체는 `packages/react/test/mount-editor.test.tsx`의 `"편집 영역을 contenteditable 속성으로 노출한다"`가 고정한다. 이 5개 안에서도 `table-selection-toolbar.test.tsx`는 18건 중 6건(셀 범위 선택 — CellSelection이 필요한데 ADR-0002가 `packages/react`의 `@tiptap/pm` 의존을 금지한다)만 fake 레인에 남았다. 층위 1 밖의 `link-toolbar.test.tsx`(12건), `formatting-toolbar.test.tsx`(10건), `table-cell-format-menu.test.tsx`(6건), `editor-content.test.tsx`(6건)는 애초에 이번 이관 대상이 아니라 전체가 fake 컨트롤러 위에서 그대로 돈다. 즉 fake 레인은 이 4개 파일 34건 + 위 6건 잔여로 총 40건이고, 위 예방 규칙(속성 대입)은 지금도 이 40건 전체에 그대로 적용된다.
