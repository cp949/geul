...
`@cp949/geul-react` 공개 표면 10개 항목을 단순 -> 복잡 순으로 1:1 매핑한다. 각 단계는 이전 단계에 컴포넌트 하나만 더한다.
...
각 예제는 정적 렌더로 끝나지 않고 상태 변화(revision, 에러/경고 등)를 화면에 노출한다 — 에이전트가 상호작용 결과를 브라우저에서 관찰할 수 있어야 한다는 §1의 목적 3을 만족하기 위한 작성 규칙이다.
...
최초 도입 시점엔 자동 e2e 회귀 게이트에 넣지 않는다. `pnpm verify`/`pnpm test:e2e`에 편입하지 않고 `pnpm --filter @cp949/geul-showcase dev`로 단독 구동한다.
