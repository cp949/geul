...
**남은 위험(해소됨, Issue #117).** `import-html.ts`(Issue #115)의 `cellRowWeight`가 이번에 걷어낸 것과 같은 "자기 자신의 rowSpan 값 자체로 가중"하는 방식을 그대로 쓰고 있어 같은 BLOCKER를 안고 있을 가능성이 높다고 남겼던 위험은 아래 Issue #117 문단에서 재현 확정하고 해소했다.

구현 반영(import-html.ts cellRowWeight 통일, Issue #117): 위 문단이 예측한 대로 `import-html.ts`의 `parseTable`도 같은 BLOCKER를 갖고 있는지 재현 확인했다 — **재현 확정**. 재현 입력 `<table><tbody><tr><td rowspan="2" colspan="3">X</td></tr><tr></tr></tbody></table>`(수정 전, 두 번째 `<tr>`가 완전히 비어 뒷받침 없음)을 `importHtml`에 넣으면 `ok: true`(`columnCount: 3`)로 오탐 통과했다 — `cellRowWeight`가 여전히 `layoutRowSpan(cell.rowSpan)` **값 자체**로 가중해, rowSpan이 덮는 두 번째 행이 완전히 비어 있어도 셀 자신의 rowSpan만으로 "혼자 주장"이 아닌 것으로 위장했기 때문이다(Issue #116이 clipboard 쪽에서 발견·수정한 것과 정확히 같은 결함이 원본 격인 `import-html.ts`에 먼저 있었고, 그 이식본이 clipboard 쪽에도 그대로 옮겨졌던 셈이다).

수정: `clipboard-table-parser.ts`가 Issue #116에서 정착한 `hasIndependentRowBacking` 패턴(rowSpan이 덮는 다른 행에 자기 자신이 아닌 다른 셀이 실제로 있을 때만 가중치 2, 없으면 1)을 `import-html.ts`의 기존 구조에 맞게 이식했다 — clipboard 쪽 코드를 그대로 복사하지 않고 차이 두 가지는 유지했다: (1) 원본은 `Result` 실패를 반환하지만 `parseTable`은 기존 관례대로 `HtmlDocumentInvalidError`를 throw한다(Issue #115가 이미 정한 어댑트 지점), (2) `columnSpanBoundFor`는 여전히 `cols.length`를 상한 후보에 포함하지 않는다 — 이 선제 검사 전체가 `cols.length === 0`(colgroup 없음) 조건 안에서만 돌기 때문이며(Issue #115 결정), 이번 변경은 이 비대칭을 건드리지 않는다. `html-round-trip/`에 두 테스트를 추가해 고정했다: "[Issue #117] 뒷받침 없이 rowSpan만 걸린 셀의 과대 colspan은 여전히 거절한다"(재현 입력, 수정 전 RED → 수정 후 GREEN), "[Issue #117] 뒷받침 있어도 실제 행 수를 넘는 위조 rowSpan은 그리드 검증이 거절한다"(`rowspan="10"`, 실제 `<tr>` 2개, 두 번째 행에 진짜 다른 셀 `C`가 있어 선제 검사는 통과하지만 model의 `validateGridCoverage`가 `SPAN_OUT_OF_BOUNDS`로 최종 거절 — 완료 조건 4 고정, 이 테스트는 수정 전에도 통과했다). 기존 "rowSpan으로 여러 행에 걸친 셀의 정당한 colspan은 오탐 거절되지 않는다"(Issue #115 F1 대상)와 "실제 행 수를 넘는 위조 rowSpan은 선제 검사를 우회해도 그리드 검증이 거절한다" 테스트는 회귀 없이 그대로 통과한다.
