...
- 인접 리프가 `divider`·`image`·`video`·`audio`·`file`(비-table atom)이면 그 자리에 그대로 두고 건너뛰어 그 너머의 병합 가능한 텍스트 블록과 현재 블록을 결합한다(Issue #202, RD-002) — 연속된 atom도 재귀적으로 전부 건너뛴다(방향 무관 대칭). 병합 자체는 위 154 문단과 같되, atom은 제거·이동 대상이 아니라 원래 있던 자리에 그대로 남는다. 별도 재귀 구현이 아니라 병합 대상을 찾는 기존 탐색이 텍스트 전용 모드에서 atom을 원래부터 건너뛰던 동작을 그대로 쓴다.
- 인접 리프가 `table`이면 여전히 그 너머의 텍스트와 병합하지 않는다(RD-002 제외 범위, RD-003 예정). 첫 Backspace/Delete는 표 전체를 `CellSelection`으로 선택하는 selection-only 동작이고, 같은 키를 한 번 더 누르면 표만 삭제하며 이 삭제가 undo 1회 단위다. atom 스킵이 table에 닿으면(예: divider 다음이 table) 그 즉시 이 표 전체 선택 규칙으로 넘어간다 — table 너머로는 계속 건너뛰지 않는다. 중첩 위치에서도 형제 컨테이너나 다른 자식을 함께 선택·삭제하지 않는다.
