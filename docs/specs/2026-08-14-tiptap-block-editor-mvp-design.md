...
- `Enter`는 블록을 분할한다. 표 셀 안은 예외다 — 셀 Enter 계약은 7.2가 소유한다.
...
- 셀 안 `Enter`는 아래 행 같은 열 셀로 캐럿을 이동한다. 선택 형태는 `Tab` 이동과 동일하다(대상 셀 내용 선택). 마지막 행에서는 아무 일도 하지 않는다.
- 표 안 `Enter`는 셀 범위 선택 중을 포함해 항상 소비된다 — 블록 분할(7.1)로 넘어가지 않고 셀·행을 분할하지 않는다. 셀 범위 선택 중에는 뒤쪽 기준 셀의 아래 행 같은 열 셀로 캐럿이 이동하며 범위 선택은 해제된다.
- 셀 안 `Shift+Enter`는 캐럿·텍스트 범위 선택 위치에 줄바꿈(hardBreak)을 삽입한다(Issue #177, #134 no-op 계약을 대체). 셀 범위 선택(여러 셀을 덮는 선택) 중에는 여전히 소비만 하고 아무 일도 하지 않는다.
