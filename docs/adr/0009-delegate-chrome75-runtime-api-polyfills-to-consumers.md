...
- `check:escompat`은 Geul 자기 소스 게이트다 — `packages/{core,io,model,react}/dist`의 JS 전량이 대상이고 `verify:packages`의 build 뒤에 돈다. 디펜던시 코드는 이 게이트의 대상이 아니다(사용처 책임이므로).
