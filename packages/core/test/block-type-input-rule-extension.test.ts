/**
 * BlockTypeInputRuleExtension이 heading(1~6)·quote·divider native
 * markdown-style shorthand를 정확 일치에서만 변환하는지, blockId·caret·
 * 즉시 Backspace 복원 계약을 지키는지 검증한다. divider는 content 없는
 * 비포장 atom이라 구조적 치환(caret 배치·trailing paragraph)까지 고정한다.
 * 즉시 Backspace 복원은 이 확장이 아니라 ListInputRuleExtension의 전역
 * undo-bridge가 처리한다(list-input-rule-extension.test.ts와 같은 계약 —
 * dispatchTextInput/typeNativeText는 block-test-support.ts로 옮긴
 * 공유 헬퍼다).
 */
