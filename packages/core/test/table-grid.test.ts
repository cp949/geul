/**
 * TableGrid의 순수 함수 계약을 확인한다 — 논리 격자 투영, 셀 범위 판정,
 * 행/열 삽입·삭제·이동, 셀 병합·분할, 헤더 행·열 토글, 행/열/셀 id
 * 단위 색상·정렬 설정, 열 너비 검증·조절을 다룬다. 표 안에 데이터를
 * 붙여넣는 pasteInto 계약은 table-grid-paste.test.ts로 분리했다.
 */
