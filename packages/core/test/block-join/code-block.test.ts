 ...
 * Delete 병합을 고정한다(Issue #202 RD-001-DELTA-01, 스펙 표 행1·2).
 *
 * 이 파일 이전에는 `block-join-extension.ts`의 `selectAdjacentCodeBlock`이
 * caret만 CodeBlock 경계로 옮기고 병합하지 않았다(select-only) — 이 파일이
 * 고정하는 병합 계약이 그 동작을 대체한다. CodeBlock은 항상 흡수되는
 * 쪽이고 대상 Text 블록이 살아남는다(코드 블록 자신의 경계 no-op·빈
 * CodeBlock Delete 삭제는 이 파일이 다루지 않는다 — code-block-exit-
 * extension.test.ts 소관).
 *
