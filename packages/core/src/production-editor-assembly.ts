...
// io.importHtml이 own-content로 재인식할 수 있도록 data-geul-* attribute로
...
// dataGeulCollapsed(<details>, "정의된 경우만 출력")와 같은 패턴으로 값이
// 있을 때만 data-geul-start-number를 낸다. io.importHtml의
  ...
  renderHTML({ node, HTMLAttributes }) {
    return [
      ...
      mergeAttributes(HTMLAttributes, {
        "data-geul-numbered-list-item": "",
        ...(node.attrs.startNumber === null
          ? {}
          : { "data-geul-start-number": String(node.attrs.startNumber) }),
      }),
      0,
    ];
  },
...
// 필수 필드(부재 상태가 없다)라 own-export(<li data-geul-checked>,
  ...
  renderHTML({ node, HTMLAttributes }) {
    return [
      ...
      mergeAttributes(HTMLAttributes, {
        ...
        "data-geul-checked": String(Boolean(node.attrs.checked)),
      }),
      0,
    ];
  },
    ...
    return [
      ...
      mergeAttributes(HTMLAttributes, {
        ...
        ...(node.attrs.collapsed === null
          ? {}
          : { "data-geul-collapsed": String(node.attrs.collapsed) }),
      }),
      0,
    ];
  },
