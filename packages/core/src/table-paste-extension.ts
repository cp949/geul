...
// CELL_LIMIT_EXCEEDED, PASTE_TARGET_NOT_FOUND 등)이든 마찬가지다. 기본
// 붙여넣기로 넘기면 TSV는 preserveWhitespace 파싱을 타서 탭이 그대로 문서에
// 들어가고(readEditorDocument가 TypeError로 터져 모델↔에디터 영구 desync),
// HTML은 표 구조가 소실된 텍스트로 뭉개진다 — 둘 다 "전체 거부" 계약 위반이다.
// 거절된 명령은 아무것도 dispatch하지 않으므로 문서·selection·stored mark가
// 그대로 보존된다(PIT-0003).
      ...
      new Plugin({
        props: {
          handlePaste: (_view, event) => {
            ...
            pasteTabularData(editor, parsed.value, createId);
            return true;
          },
        },
      }),
