...
describe("체크 목록 HTML 내보내기", () => {
  it("checked true/false 항목을 ul과 data-geul-checked 속성으로 내보낸다", () => {
    ...
    expect(exportHtml(document)).toEqual({
      ok: true,
      value:
        '<ul><li data-geul-block-id="c-1" data-geul-checked="true">완료</li>' +
        '<li data-geul-block-id="c-2" data-geul-checked="false">미완료</li></ul>',
    });
  });
  ...
  it("bulletListItem과 checkListItem이 인접해도 종류별로 다른 ul로 나눈다", () => {
    ...
    expect(exportHtml(document)).toEqual({
      ...
      value:
        ...
        '<ul><li data-geul-block-id="c-1" data-geul-checked="false">체크</li></ul>',
    });
  });
});
