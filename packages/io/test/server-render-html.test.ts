/**
 * 순수 Node(서버) 환경에서 DOM 전역 없이 exportHtml/importHtml 왕복이
 * 동작함을 고정하는 회귀 테스트. `IO-009`(Issue #156 슬라이스9) 판정 근거.
 * `packages/io/tsconfig*.json`은 DOM lib을 포함하지 않아 bare `document`
 * 식별자 자체가 컴파일되지 않는다 — 여기서는 `globalThis` 인덱싱으로
 * 런타임에 DOM 전역이 실제로 없는지 확인한다.
 */
...
describe("서버 환경 HTML 변환", () => {
  ...
  it("문서를 HTML로 내보내고 다시 가져와도 블록 내용이 보존된다", () => {
    const exported = exportHtml(sampleDocument);
