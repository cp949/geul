 ...
 * roadmap-workflow RD-001 DELTA-02)을 검증한다. GFM(mdast)의 blockquote
 * 노드는 root와 같은 flow content를 자식으로 가질 수 있어(DELTA-01 실측)
 * quote의 children(재귀, 임의 깊이·임의 block 타입)을 blockquote 중첩으로
 * 그대로 표현한다 — strict export가 성공하고 lossy export도 경고 없이 같은
 * 구조를 낸다. 유일한 예외는 목록 항목과 같은 모호성이다: own content가
 * 비고 첫 child가 paragraph면 GFM이 둘의 경계를 구분하지 못해
 * NESTED_CHILDREN을 그대로 보고한다(loss-analysis.ts의
 * hasAmbiguousLeadingListParagraph). import 쪽(quote가 markdown에서 다시
 * children을 갖도록 복원하는 것)은 DELTA-03 범위다 — 이 파일은 export(Document
 * → markdown)만 검증하고 재import는 다루지 않는다.
 */
