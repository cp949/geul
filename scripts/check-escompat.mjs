 ...
 *
 * `packages/*`의 dist JS 전량(목록은 workspace-roots 열거에서 파생)을 공식
 * browser floor Chrome 75(ADR-0008) 기준으로 검사한다. 1순위 도구였던
 * check-es-compat은 npm에서 deprecated이고 EOL인 eslint@8에 고정돼 있으며
 * ESM 산출물을 지원하지 않아(업스트림 issue #69), 계획서의 폴백대로 eslint +
 ...
 * 기능의 규칙만 개별 해제한다. 소스 문법은 tsc `target: "ES2019"`
 * (`tsconfig.base.json`; react 배포 산출물만 esbuild `--target=chrome75`를
 * 추가로 거친다)가 이미 downlevel하므로 이 게이트가 실제로 잡는 것은
 * 런타임 API 사용이다(예: Array.prototype.findLast — Chrome 97+).
 *
 * 범위: es-x는 **ECMAScript 표준** 문법·API만 검사한다. Web API 격차(예:
 * crypto.randomUUID — Chrome 92+)는 이 게이트 밖이고 Issue #121이 소유한다.
 *
 ...
 * 그래서 대상 파생과 ESLint 구성을 export하고, 직접 실행일 때만 게이트를
 * 돌린다.
 */
...
/**
 * 검사 대상 dist 디렉터리 — 배포 라이브러리(`packages/*`)의 dist 전량이다.
 * 목록은 `workspace-roots.mjs`의 열거에서 파생한다 — 패키지 목록의 단독
 * 소유 계약(`tests/workspace-roots.test.ts`)이라 로컬 리터럴 사본을 두지
 * 않는다. `apps/*`·`fixtures/*`의 dist는 디펜던시까지 번들된 앱 산출물이라
 * 대상이 아니다 — 디펜던시의 런타임 API는 사용처 core-js 책임이고
 * (ADR-0009) 이 게이트의 대상은 자기 소스뿐이다.
 *
 * @param {string} repoRoot 저장소 루트 경로
 * @returns {string[]} dist 디렉터리 절대 경로 목록(정렬됨)
 */
export const escompatDistDirectories = (repoRoot) => {
  const packagesRoot = join(repoRoot, "packages");
  return workspacePackageDirectories(repoRoot)
...
/**
 * 게이트가 쓰는 ESLint 인스턴스. `tests/check-escompat.test.ts`가 같은
 * 구성으로 위반 seed를 lint해 검출력이 살아 있는지 고정한다.
 *
 * @returns {ESLint}
 */
export const createEscompatESLint = () =>
  new ESLint({
    cwd: workspaceRoot,
...
const runGate = async () => {
  ...
  for (const directory of escompatDistDirectories(workspaceRoot)) {
  ...
  console.log(
    `[check:escompat] ${targetFiles.length}개 파일 Chrome >= 75 기준 통과` +
      " (ES 표준 문법·API 기준 — Web API 격차는 게이트 밖, Issue #121 소유)",
  );
};
