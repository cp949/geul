 ...
 * 동어반복이 되어 drift 감시가 사라진다(`G-TST-002`: 단언이 구현을 되뇌면 그
 ...
 * 없음을 증명하는 장치가 아니다.
 *
 * 그 계약 테스트를 이름으로 가리키는 것은 `G-TST-002`가 허용하는 예외다 —
 * 공용 모듈이 자기 전제를 지는 쪽을 가리키는 방향이다. 줄 번호는 인용하지
 * 않는다 — 인용 대상이 움직여도 아무것도 실패하지 않는다.
 */
import { existsSync, readdirSync, statSync } from "node:fs";
import { resolve } from "node:path";

/**
 * `<이름>/*` 형태의 glob 하나를 이름 세그먼트로 가른다. 이름 세그먼트는 슬래시도
 * glob 메타문자도 포함하지 않는 **디렉터리 이름 리터럴**이고, 그 뒤에 정확히
 * `/*`(단일 별) 하나만 와야 전체 문자열과 일치한다 — 이중 별, 이름 없는 패턴,
 * 부정 접두, 이름 뒤에 추가 세그먼트가 오는 패턴, `./` 접두 패턴은 전부 이
 * 형태가 아니다.
 *
 * 이름 세그먼트를 "슬래시가 없는 임의의 문자열"로 열어 두면 안 된다(실측).
 * 그렇게 두면 이름 자리가 `*` 또는 `**`인 glob — pnpm이 "모든 최상위
 * 디렉터리 아래의 패키지"로 정상 해석하는 형태 — 이 그대로 통과하고,
 * `resolve(repoRoot, "*")`는 그 이름의 디렉터리 리터럴을 가리켜 트리에
 * 존재하지 않는다. 아래 `existsSync` 가드가 그것을 조용히
 * 건너뛰어 열거가 통째로 0이 되고, `scripts/check-licenses.mjs`가
 * `0 manifests`로 exit 0을 낸다 — 이 모듈이 막으려는 "조용히 좁아진 감시
 * 장치" 그 자체다. 그래서 메타문자가 든 이름 세그먼트는 통과시키지 않고
 * 시끄럽게 throw한다.
 *
 * @type {RegExp}
 */
 ...
 * 필터 없는 이 열거를 자기 루프로 다시 구현한 세 번째 사본이었다(`G-TST-002`).
 *
