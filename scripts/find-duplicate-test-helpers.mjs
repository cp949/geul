/**
 * 테스트 파일 사이에 복제된 헬퍼를 본문 기준으로 찾는다(`G-TST-002`).
 *
 * 이름이 아니라 정규화한 **본문 해시**로 비교한다. Issue #92 이전의 검증
 ...
 * ## 오탐 기준
 *
 * 정규화한 본문이 **2줄 이상**인 선언만 비교한다. `G-TST-002`가 막으려는
 ...
 *   나눗셈으로 읽는다.
 * - **본문 정규화가 줄바꿈을 보존한다.** 줄마다 여백은 지우지만 개행은
 *   남기므로, 같은 헬퍼라도 객체 리터럴을 한 줄로 쓴 사본과 여러 줄로 쓴
 *   사본이 다른 해시를 받는다. biome이 두 형태를 다 통과시켜 `pnpm lint`가
 *   차이를 없애 주지도 않는다. Issue #91에서 `editor-controller-table.test.ts`의
 *   `editorWithTable` 두 벌이 `return` 객체 확장만 다른 진짜 중복이었는데 이
 *   이유로 그룹이 되지 않았다(실측: 한쪽 `return`을 상대 형태로 펴면 해시가
 *   같아진다). 개행까지 접는 수정은 서로 다른 헬퍼를 붙이는 오탐을 늘리는
 *   방향이라 그 자체로 별도 판단이 필요하다.
 *
 * ## 대상 목록을 좁히지 않기
 *
