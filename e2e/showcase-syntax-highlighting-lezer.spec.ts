/**
 * RD-003-DELTA-04(BLK-017, Issue #162) — `apps/showcase`의
 * CodeMirror(`@lezer/javascript` + `@lezer/highlight`) 구문 강조 예제를
 * 검증한다. 페이지 이동은 `e2e/support/showcase.ts`의
...
test("lezer 예제는 코드 블록에 토큰별 class를 렌더하고 source 텍스트를 바꾸지 않는다", async ({
  page,
}) => {
  const code = await openShowcaseExample(
    page,
    "/examples/syntax-highlighting-lezer",
  );
  ...
  // `@lezer/highlight`의 `classHighlighter`가 만드는 표준 class 접두사
  // (`tok-`)를 가진 span이 하나 이상 있어야 한다 — `syntaxHighlighter`가
  // 실제로 연결됐다는 증거다.
  const highlightedSpans = code.locator('span[class*="tok-"]');
  ...
  // theme.css `.tok-keyword` 색상과 일치하는지 구체적으로 확인한다 —
