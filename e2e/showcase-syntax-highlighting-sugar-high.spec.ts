/**
 * RD-003-DELTA-05(BLK-017, Issue #162) — `apps/showcase`의 sugar-high
...
test("sugar-high 예제는 코드 블록에 토큰별 class를 렌더하고 source 텍스트를 바꾸지 않는다", async ({
  page,
}) => {
  const code = await openShowcaseExample(
    page,
    "/examples/syntax-highlighting-sugar-high",
  );
  ...
  // sugar-high `generate()`가 만드는 표준 class 접두사(`sh__token--`)를
  // 가진 span이 하나 이상 있어야 한다 — `syntaxHighlighter`가 실제로
  // 연결됐다는 증거다.
  const highlightedSpans = code.locator('span[class*="sh__token--"]');
  await expect(highlightedSpans.first()).toBeVisible();
  expect(await highlightedSpans.count()).toBeGreaterThan(0);

  // 알려진 keyword 토큰("function")의 computed color가 이 예제의
  // theme.css `.sh__token--keyword` 색상과 일치하는지 구체적으로
  // 확인한다 — class 존재만으로는 어댑터가 엉뚱한 토큰에 우연히 class를
  // 붙인 false positive를 배제하지 못한다.
  const functionKeyword = code.getByText("function", { exact: true });
  await expect(functionKeyword).toHaveCSS("color", "rgb(207, 34, 46)");
});
