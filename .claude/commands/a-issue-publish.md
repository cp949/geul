...
작업 폴더 해석은 `AGENTS.md`의 "로컬 작업공간"을 따른다.

**`final-report.md`가 있는 작업 폴더의 초안만 등록한다.** 등록은 상태 전이의 마지막 단계이고, `AGENTS.md`가 "각 단계는 앞 단계 산출물이 있어야 시작한다"로 정한 앞 단계 산출물이 `final-report.md`다. 없이 등록하면 두 가지가 깨진다.
