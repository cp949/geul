...
4. 첫 커밋이 필요한 시점에 `git switch -c <type>/$1-<slug> dev`로 브랜치를 만들고 `meta.md`에 기록한다. 분기 기준을 `dev`로 명시한다 — 현재 브랜치가 `dev`라고 가정하면 다른 작업 브랜치 위에 쌓여 `/merge-dev`의 ff-only가 남의 커밋을 함께 끌고 간다. 앞 작업의 산출물이 필요한 의존 작업일 때만 그 브랜치에서 분기하고 의존을 `meta.md`에 적는다. worktree를 만들지 않고 subagent에도 worktree 격리를 주지 않는다.
