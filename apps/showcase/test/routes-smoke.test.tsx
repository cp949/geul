...
/**
 * exampleRoutes에 등록된 11개 예제 페이지 전부가 MemoryRouter 안에서
...
describe("exampleRoutes 전체 스모크", () => {
  it("정확히 11개 예제가 등록돼 있다", () => {
    expect(exampleRoutes).toHaveLength(11);
  });
  ...
  it("모든 예제 경로가 서로 다르다", () => {
    expect(new Set(exampleRoutes.map((route) => route.path)).size).toBe(11);
  });
